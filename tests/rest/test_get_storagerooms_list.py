...
@mock.patch(
    'ddd_attempt1.use_cases.storageroom_use_cases.StorageRoomListUseCase')
...
def test_get_failed_response(mock_use_case, client):
    ...
    assert json.loads(http_response.data.decode('UTF-8')) == \
        {'type': res.ResponseFailure.SYSTEM_ERROR, 'message': 'test message'}
    assert http_response.status_code == 500
    assert http_response.mimetype == 'application/json'


@mock.patch('ddd_attempt1.use_cases.storageroom_use_cases.StorageRoomListUseCase')
def test_request_object_initialisation_and_use_with_filters(mock_use_case, client):
    ...
    request_object_class = 'ddd_attempt1.shared.request_object.StorageRoomListRequestObject'
